...
const isProductExist = async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    const isCollection = await productRepositories.findItemByAttributes(
      Collection,
      "id",
      collectionId
    );
    ...
    const isProductAvailable =
      await productRepositories.findByModelAndAttributes(
        Products,
        "name",
        "sellerId",
        req.body.name,
...
const isProductOwner = async (req: any, res: Response, next: NextFunction) => {
  try {
    const sellerId = req.user.id;
    const productId = req.params.id;
    const product = await productRepositories.findProductByIdAndSeller(
      productId,
      sellerId
    );
    if (!product) {
...
const isProductNotExist = async (
  ...
  try {
    const productId = req.params.id;
    const isProductAvailable = await productRepositories.findItemByAttributes(
      Products,
      "id",
      productId
    );
    if (!isProductAvailable) {
      return res.status(httpStatus.NOT_FOUND).json({
        status: httpStatus.NOT_FOUND,
        message: "Product does not exist in your collection",
      });
    }
    next();
  } catch (error) {
...
const isCollectionExist = async (
  ...
  try {
    const isCollection = await productRepositories.findByModelAndAttributes(
      Collection,
      "name",
      "sellerId",
      req.body.name,
