...
import { validation, isProductExist, isShopExist, transformFilesToBody, isSellerShopExist,isProductsByCategorySelected } from "../middlewares/validation";
import { shopSchema, productSchema, statisticsSchema, statusSchema } from "../modules/product/validation/productValidation";
...
router.put("/seller-update-product-status/:id", userAuthorization(["seller"]), validation(statusSchema), productController.updateProductStatus);
router.get("/seller-get-products", userAuthorization(["seller"]), isSellerShopExist ,productController.sellerGetProducts);
router.get("/user-get-products",isProductsByCategorySelected,productController.userGetAvailableProducts);
