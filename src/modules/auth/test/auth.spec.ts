...
describe("isUserProfileComplete Middleware", () => {
  ...
  it("should return 400 if user profile is incomplete", async () => {
    ...
    await isUserProfileComplete(req as Request, res as Response, next);
    expect(
      (res.status as sinon.SinonStub).calledOnceWith(httpStatus.BAD_REQUEST)
    ).to.be.true;
  ...
  it("should return 500 on internal server error", async () => {
    ...
    expect(
      (res.status as sinon.SinonStub).calledOnceWith(
        httpStatus.INTERNAL_SERVER_ERROR
      )
    ).to.be.true;
    expect((res.json as sinon.SinonStub).calledOnce).to.be.true;
...
describe("Submit Seller Request Test Case", () => {
  ...
  it("should submit a seller request successfully", (done) => {
    if (!buyerToken) {
    ...
    router()
      ...
      .end((err, res) => {
        ...
        expect(res.body).to.have.property("message", "Seller request submitted successfully");
        expect(res.body.data).to.be.an("object");
        expect(res.body.data.sellerRequest).to.have.property("id");
        expect(res.body.data.sellerRequest).to.have.property("userId");
        expect(res.body.data.sellerRequest).to.have.property("requestStatus", "Pending");
        expect(res.body.data.sellerRequest).to.have.property("createdAt");
        expect(res.body.data.sellerRequest).to.have.property("updatedAt");
        done(err);
      });
  });

  it("should return 400 if Seller request already submitted", (done) => {
    router()
      .post("/api/user/user-submit-seller-request")
      .set("Authorization", `Bearer ${buyerToken}`)
      .end((error, response) => {
        ...
        expect(response.body).to.have.property("message", "Seller request already submitted");
        done(error);
      });
  });
});
