...
import * as sendEmail from "../../../services/sendEmail";
...
describe("verifyUserCredentials Middleware", () => {
  ...
  it("should call next if user is found and password matches and 2FA is not enabled", async () => {
    const user: any = {
      id: 1,
      email: req.body.email,
      password: "hashedPassword",
...
it("should send OTP email if 2FA is enabled", async () => {
  const user:any = {
    id: 1,
    email: "user@example.com",
    password: "hashedPassword",
  ...
  sinon.stub(authRepositories, "findUserByAttributes").resolves(user);
  sinon.stub(helpers, "comparePassword").resolves(true);
  sinon.stub(authRepositories, "createSession").resolves(session);
  sinon.stub(authRepositories, "findTokenByDeviceIdAndUserId").resolves(null);
  sinon.stub(helpers, "generateRandomCode").returns("123456");
  sinon.stub(sendEmail, "sendVerificationEmail").resolves();

  req.headers["user-device"] = "device123";

  await verifyUserCredentials(req, res, next);

  expect(req.user).to.deep.equal(user);
  expect(authRepositories.createSession).to.have.been.calledWith(
    sinon.match({
      userId: user.id,
      device: "device123",
      otp: "123456"
  ...
  expect(sendEmail.sendVerificationEmail).to.have.been.calledWith(
    user.email,
